...
public interface NotaCreditoRepository extends JpaRepository<NotaCredito, Long>, JpaSpecificationExecutor<NotaCredito> {
    ...
    @Query(
        value = "select distinct notaCredito from NotaCredito notaCredito left join fetch notaCredito.facturas",
        countQuery = "select count(distinct notaCredito) from NotaCredito notaCredito"
    )
    Page<NotaCredito> findAllWithToOneRelationships(Pageable pageable);

    @Query("select distinct notaCredito from NotaCredito notaCredito left join fetch notaCredito.facturas")
    ...
    List<NotaCredito> findByFechaBetween(LocalDate p1, LocalDate p2);
}
