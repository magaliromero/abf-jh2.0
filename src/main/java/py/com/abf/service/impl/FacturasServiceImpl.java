...
/**
 * Service Implementation for managing {@link py.com.abf.domain.Facturas}.
 */
@Service
@Transactional
public class FacturasServiceImpl implements FacturasService {
     ...
     */
    @Transactional(readOnly = true)
    public List<Facturas> findAllWhereNotaCreditoIsNull() {
        log.debug("Request to get all facturas where NotaCredito is null");
        return StreamSupport
            .stream(facturasRepository.findAll().spliterator(), false)
            .filter(facturas -> facturas.getNotaCredito() == null)
            .toList();
    }
